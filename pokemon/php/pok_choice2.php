...
if(isset($_GET["username"]) && isset($_GET["password"])) {
    ...
    $donnee = $req->fetch();
        ...
        if ($_GET["username"] != $donnee["username"] && $_GET["password"] != $donnee["password"])
            {
                header('Location:testUsers2.php');
            }
        else
            {
            if(empty($_SESSION["username2"])) {
                $_SESSION["username2"]  = $_GET["username"];//Passer en POST une fois login pret et fonctionnel!!!
            }
        ...
        if (empty($_SESSION["username2"])){ //si il a pas de username en venant de testUsers.php je mets un GET pour les tests une fois fini le passer en POST

            echo "Veuillez vous inscrire afin de pouvoir jouer";
        }
        elseif(isset($_SESSION["username2"])){ //si il a un username
        ?>
                ...
                <div>
                    <table>
                        ...
                        while ($donnee = $reponse->fetch()){    // on affiche la reponse .......
                            ...
                            echo'<tr><td><input type=checkbox name="pokemon2[]" value="' . $donnee["pok_name"] . '"/></td>';
                ...
                <div>
                    ...
                    <table>
                        ...
                        while ($donnee = $reponse->fetch()){    // on affiche la reponse .......
                            ...
                            echo'<tr><td><input type=checkbox name="pokemon2[]" value="' . $donnee["pok_name"] . '"/></td><td><p>' . strtolower($donnee["pok_name"]) . '</p></td><td><img  src='. $donnee["urlimage"] .'></td></tr>';
